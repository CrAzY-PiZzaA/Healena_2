    ...
    if current_state == FSM.enter_stat:
        ...
        for st in lst:
            ...
            if flag_presence:
                ...
                await message.reply("Похоже, что продукта " + prod +
                " нет в нашей базе данных или вы написали его название с ошибками\n\nЕсли все же написали правильно, попробуйте вместо него написать один из указанных ниже продуктов, наиболее близкий к вашему\n\n")
                for uniduct in await orm_get_uniducts(session):
                    if cat == uniduct.category:
                        await message.answer(str(uniduct.name) + "\n")
                await message.answer("\n")
        if not flag:
